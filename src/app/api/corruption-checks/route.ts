...
import { corruptionError, corruptionInclude, officialSearch, saveCorruptionCheck, resolveOfficial } from '@/lib/corruption-server'
...
export async function GET(req: Request) {
  try {
    ...
    if (q.officialId) q.officialId = (await resolveOfficial(prisma, q.officialId))?.id ?? q.officialId
    const where: Prisma.CorruptionCheckWhereInput = {
      ...
      AND: [
        ...
        ...(q.search ? [{ OR: [{ official: { OR: [officialSearch(q.search), { mergedFrom: { some: officialSearch(q.search) } }] } }, { findings: { contains: q.search } }, { location: { contains: q.search } }] }] : []),
      ],
    }
