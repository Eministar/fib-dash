...
export async function POST() {
  try {
    ...
    const result = await applyChangeHistory(user.id, 'redo')
    await queueAffectedSyncs(result.affectedModels).catch((syncError) => {
      console.error('[change-history] Resync nach Redo fehlgeschlagen:', syncError)
    })
