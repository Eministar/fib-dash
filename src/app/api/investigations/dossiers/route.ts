...
export async function GET(req: Request) {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.dossier.findMany({ where, take: 30, skip: (query.page - 1) * 30, orderBy: [{ updatedAt: 'desc' }, { id: 'asc' }], include: { photo: { select: { id: true, title: true } }, createdBy: { select: { id: true, displayName: true } }, _count: { select: { persons: true, vehicles: true, mapSpots: true, investigations: { where: investigationVisibilityWhere(user) } } } } }),
