...
export async function GET(req: Request) {
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.dossier.findMany({ where, take: 30, skip: (query.page - 1) * 30, orderBy: [{ updatedAt: 'desc' }, { id: 'asc' }], include: { photo: { select: { id: true, title: true } }, parent: { select: { id: true, title: true } }, _count: { select: { children: true, persons: true, vehicles: true, investigations: { where: investigationVisibilityWhere(user) }, clips: { where: { investigation: investigationVisibilityWhere(user) } } } } } }),
