...
function ShareEditor({ existing, onClose, onSaved }: { existing?: Share; onClose: () => void; onSaved: (path?: string) => void }) {
  ...
  return <Modal open onClose={loading ? () => {} : onClose} title={existing ? 'Freigabe bearbeiten' : 'Freigabelink erstellen'} size="xl"><form className="space-y-4" onSubmit={async e => {
    ...
    <section className="space-y-2 rounded-lg border border-[#343434] p-3"><h3 className="text-sm font-semibold text-white">{selected.length} von maximal 100 Einträgen ausgewählt</h3><p className="text-xs text-[#909090]">Nur diese Inhalte werden veröffentlicht. Fotos der gewählten Akten und die Chronologie gewählter Einsatzakten gehören dazu. Einsatzakten, Personen, Fahrzeuge und Clips einer Dauerakte zusätzlich auswählen.</p><div className="max-h-40 space-y-1 overflow-auto">{selected.map(item => <div key={itemKey(item)} className="flex items-center justify-between gap-2 text-xs text-[#c4c4c4]"><span>{SHARE_KINDS[item.kind]} · {item.title}{item.classifiedAtGrant ? ' · Verschlusssache' : ''}</span><Button type="button" variant="ghost" size="sm" onClick={() => setSelected(selected.filter(i => itemKey(i) !== itemKey(item)))}>Entfernen</Button></div>)}</div></section>
